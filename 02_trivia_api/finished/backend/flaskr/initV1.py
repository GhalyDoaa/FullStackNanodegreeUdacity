...
def create_app(test_config=None):
    ...
    def delete_question():
        ...
        try:
            question = Question.query.filter(Question.id == q_id).all()
            if question is None:
                abort(404)
            question.delete()
    ...
    def play_quiz():
        ...
        print("=======================================")
        print("previous_questions", previous_questions, "current_category", current_category)
        print("type of previous_questions", type(previous_questions))

        if not previous_questions:
            if current_category:
                print("==============hello from if 1=========================")
                selection = Question.query.filter(Question.category_id == current_category).all()
                print("=======================================", "len of selection", len(selection))
            else:
                print("==============hello from else 1=========================")

                selection = Question.query.all()
        else:
            if current_category:
                print("==============hello from if 2=========================")
                # testby
        ...
        # Qis=Qobject.get("question")
        # return jsonify({
        #     'question': Qis,
        #     'message': 'hello from play'
        #
        # })
        return jsonify({
            'success': True,
